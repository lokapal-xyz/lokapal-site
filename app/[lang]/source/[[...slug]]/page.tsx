...
export default async function DocsPage(props: PageProps) {
  ...
  // 🔑 pass "Source Agents" explicitly as the section
  const res = await getDocsForSlug("source", slugPath, lang);

  if (!res) notFound();

  // Construct the full pathname that getPreviousNext expects
  const fullPathname = `/${lang}/source/${slugPath}`;
      ...
      <div className="flex-[4.5] pt-10">
        <Typography>
          ...
          <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-800">
            <Pagination pathname={fullPathname} dict={dict} section="source" lang={lang}/>
          </div>
        </Typography>
      </div>
      <Toc section="source" path={slugPath} dict={dict} lang={lang}/>
    </div>
  );
}
...
// In source/[[...slug]]/page.tsx
export function generateStaticParams() {
  return getPageRoutesForSection("source").map((item) => ({
