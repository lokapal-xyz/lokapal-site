...
export default async function DocsPage(props: PageProps) {
  ...
  // 🔑 pass "devotion" explicitly as the section
  const res = await getDocsForSlug("devotion", slugPath, lang);

  if (!res) notFound();

  // Construct the full pathname that getPreviousNext expects
  const fullPathname = `/${lang}/devotion/${slugPath}`;
...
export function generateStaticParams() {
  return getPageRoutesForSection("devotion").map((item) => ({
