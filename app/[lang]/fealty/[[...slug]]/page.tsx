...
export default async function DocsPage(props: PageProps) {
  ...
  // 🔑 pass "fealty" explicitly as the section
  const res = await getDocsForSlug("fealty", slugPath, lang);

  if (!res) notFound();

  // Construct the full pathname that getPreviousNext expects
  const fullPathname = `/${lang}/fealty/${slugPath}`;
      ...
      <div className="flex-[4.5] pt-10">
        <Typography>
          ...
          <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-800">
            <Pagination pathname={fullPathname} dict={dict} section="fealty" lang={lang}/>
          </div>
        </Typography>
      </div>
      <Toc section="fealty" path={slugPath} dict={dict} lang={lang}/>
    </div>
  );
}
...
export function generateStaticParams() {
  return getPageRoutesForSection("fealty").map((item) => ({
