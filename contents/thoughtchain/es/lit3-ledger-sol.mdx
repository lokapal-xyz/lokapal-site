...
El campo `contentHash` es opcional y se alinea con el estándar **HNP-1 (Protocolo de Normalización de Hash)** descrito en la serie de ensayos de Lit3. 
...
Devuelve el struct `Entry` completa en el índice especificado. Se revierte si el índice no existe.
...
Almacena la dirección en espera de aceptación de la autoridad de curador.
...
**Paso 2:** El curador propuesto debe aceptar explícitamente llamando a esta función desde su billetera. Solo después de la aceptación se completa la transferencia.
...
Se revierte cuando una cuenta no autorizada intenta aceptar la transferencia.
...
3. **Resultado:** Si `0xNewAddress` nunca llamó a la función correspondiente, la autoridad de curador permanece sin cambios.
...
| `Lit3Ledger__NotZeroAddress()` | Una dirección es cero cuando debe ser válida. |
...
El Lit3Ledger.sol actual es intencionalmente mínimo, enfocándose principalmente en archivado y versionado. Las extensiones futuras podrían incluir:
...
- **Almacenamiento:** Cada entrada añade aproximadamente 2000-2500 gas por escritura (dependiendo de las longitudes de las strings).
- **Consultas:** Las funciones de solo lectura son costo cero.
