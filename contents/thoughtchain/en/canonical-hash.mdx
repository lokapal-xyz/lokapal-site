---
title: "Notes on Lit3 — Part 4: Lit3 Canonical Hash"
...
- [*Lit3 Frameworks*](https://www.lokapal.xyz/en/thoughtchain/lit3-frameworks), and
...
The Canonical Hash is generated using a **cryptographic hash function**, specifically **SHA-256**, which is implemented on the Ethereum Virtual Machine (EVM) as a precompiled contract.
...
1.  **Preparation (Off-Chain):** The Lit3 author or platform takes the final text file. They run the **SHA-256** algorithm over the entire contents to generate the 32-byte hash.
...
3.  **The Proof:** Once the transaction is confirmed on-chain, the `contentHash` is fixed forever. This 32-byte value is now the **mathematical representation of the canonical text.**
